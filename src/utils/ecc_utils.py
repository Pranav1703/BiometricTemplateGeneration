...
class ECCWrapper:
    ...
    def __init__(self, message_size: int = 32, error_capacity_percent: float = 0.2):
        """
        Initialize the ECC wrapper.

        Args:
            message_size (int): The size of the message in bytes (default 32 bytes for 256 bits).
        ...
        # Usually capacity is relative to the total block size or message size.
        # Let's interpret "20% error-correction capacity" as correcting 20% of the total block.
        # But commonly we define it based on message size for simple configuration.
        # If we target correcting `k` errors, we need `2*k` parity symbols.
        # Total size = message_size + 2*k.
        # If k is approx 20% of total size... 
        # Let's just fix it to be robust. 
        # For 32 bytes message, if we want to correct ~20% of the *message* size (6 bytes), we need 12 parity bytes.
        # If we want to correct ~20% of the *packet* size...
    ...
    def encode(self, data: bytes) -> bytes:
        """
        Encodes the data with Reed-Solomon error correction.
    ...
    def decode(self, data: bytes) -> bytes:
        """
        Decodes the data and corrects errors if possible.
