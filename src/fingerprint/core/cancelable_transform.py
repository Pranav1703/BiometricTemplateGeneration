...
try:
    ...
    logger.warning("cryptography not available, key derivation will use fallback")
...
class CancelableTransform:
    ...
    def __init__(self, embedding_dim: int = 512, alpha: float = 0.5):
        ...
        self.alpha = alpha
        logger.info(
            f"CancelableTransform initialized: dim={embedding_dim}, alpha={alpha}"
        )

    def _generate_projection_matrix(
        self, user_key: str, embedding: np.ndarray
    ) -> np.ndarray:
        """
    ...
    def extract_stable_bits(
        self, embedding: np.ndarray, num_bits: int = 128
    ) -> np.ndarray:
        """
        Extract stable bits from transformed embedding using quantization.

        Args:
            embedding: Transformed embedding (512-dim)
            num_bits: Number of bits to extract (default: 128)

        Returns:
            Binary array of stable bits
        """
        embedding = embedding.flatten()

        # Use fixed threshold at 0 (embeddings are centered after transform)
        threshold = 0.0

        bits = (embedding > threshold).astype(np.uint8)

        # Select bits from dimensions with highest absolute value (most discriminative)
        var_indices = np.argsort(np.abs(embedding))[-num_bits:]
        stable_bits = bits[var_indices]

        return stable_bits

    def derive_key(
        self, transformed: np.ndarray, salt: bytes, key_length: int = 32
    ) -> Tuple[bytes, bytes]:
        """
        Derive cryptographic key from transformed embedding using salt + hash.

        Uses the transformed embedding directly (which is stable under noise
        due to cosine similarity preservation in cancelable transform).
        """
        emb_bytes = transformed.tobytes()
        ...
        if CRYPTO_AVAILABLE:
            ...
            derived_key = kdf.derive(emb_bytes)
        else:
            derived_key = hashlib.pbkdf2_hmac(
                "sha256", emb_bytes, salt, 10000, dklen=key_length
            )

        key_hash = hashlib.sha256(derived_key).digest()

        return derived_key, key_hash

    def enroll_with_key(
        self, embedding: np.ndarray, user_key: str
    ) -> Tuple[Dict, Dict]:
        """
        Full enrollment with key derivation (Option A).
        """
        transformed, params = self.enroll(embedding, user_key)

        salt = os.urandom(16)

        derived_key, key_hash = self.derive_key(transformed, salt, key_length=32)

        template = {"transformed": transformed, "salt": salt, "key_hash": key_hash}

        return template, params

    def verify_with_key(
        self, embedding: np.ndarray, template: Dict, transform_params: Dict
    ) -> Tuple[bool, bytes]:
        """
        Full verification with key derivation (Option A).

        For cancelable biometrics, we use cosine similarity for verification
        (which is robust to noise) rather than exact key matching.
        """
        transformed = self.verify(embedding, transform_params)

        stored_transformed = template["transformed"]

        # Use cosine similarity for verification (robust to noise)
        similarity = np.dot(transformed, stored_transformed) / (
            np.linalg.norm(transformed) * np.linalg.norm(stored_transformed) + 1e-10
        )

        # Similarity threshold for biometric verification
        threshold = 0.5

        success = similarity >= threshold

        # Derive key for return (can be used for cryptographic operations)
        if success:
            salt = template["salt"]
            _, derived_key = self.derive_key(transformed, salt, key_length=32)
        else:
            derived_key = b""

        return success, derived_key

    def cancel(
        self, user_key_old: str, user_key_new: str, embedding: np.ndarray
        ...
        """
        logger.info(
            f"Cancelling template with key '{user_key_old}', issuing new key '{user_key_new}'"
        )
