...
from src.fingerprint.biometric_crypto_system import BiometricCryptoSystem
...
class TestCancelableTransform:
    ...
    def test_verification_same_embedding(self):
        ...
        similarity = np.dot(transformed, verified) / (np.linalg.norm(transformed) * np.linalg.norm(verified))
    ...
    def test_cancellation(self):
        ...
        # Cancel and reissue
        new_transformed, new_params = self.transform.cancel(old_key, new_key, self.embedding)
        ...
        similarity_new = np.dot(new_transformed, verified_new) / (np.linalg.norm(new_transformed) * np.linalg.norm(verified_new))
...
class TestFuzzyCommitment:
    ...
    def setup_method(self):
        """Setup test fixtures."""
        self.ecc = ECCWrapper(message_size=32, error_capacity_percent=0.2)
    ...
    def test_verification_same_embedding(self):
        """Test verification with same embedding."""
        hash_key, helper_data = self.fuzzy.enroll(self.embedding)
        success, recovered_key = self.fuzzy.verify(self.embedding, hash_key, helper_data)
    ...
    def test_verification_small_noise(self):
        ...
        success, recovered_key = self.fuzzy.verify(noisy_embedding, hash_key, helper_data)
...
class TestBiometricCryptoSystem:
    ...
    def test_verification_same_embedding(self):
        """Test verification with same embedding."""
        template = self.system.enroll(self.embedding, self.user_key)
        success, recovered_key = self.system.verify(self.embedding, template, self.user_key)
    ...
    def test_verification_noisy_embedding(self):
        ...
        success, recovered_key = self.system.verify(noisy_embedding, template, self.user_key)
    ...
    def test_cancellation_and_reissuance(self):
        ...
        assert new_template['cancelable_params']['user_key'] != old_template['cancelable_params']['user_key']
        ...
        success_new, recovered_key = self.system.verify(self.embedding, new_template, new_key)
    ...
    def test_torch_tensor_input(self):
        ...
        if TORCH_AVAILABLE:
            ...
            success, recovered_key = self.system.verify(torch_embedding, template, self.user_key)
